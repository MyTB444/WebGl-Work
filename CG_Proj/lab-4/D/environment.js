...
let draw_mode = 0;          // global variables for the button
let render_wire = false;   
// uniform locations
...
async function setup(meshes)
{
    ...
    draw_mode = gl.TRIANGLES;       // has to be assigned here as we require a gl context
...
function render_object()
{
    ...
    // D2 MODIFY HERE
    gl.drawElements(draw_mode, mesh.indexBuffer.numItems, gl.UNSIGNED_SHORT, 0);  // pass in new variable that is used with the new button
}
...
async function render() 
{
    ...
    let canvasWidthLoc = gl.getUniformLocation(program, "canvas_width"); // uniform passed here
    ...
    gl.uniform1f(canvasWidthLoc, canvas.width);//set
    gl.uniform1i(gl.getUniformLocation(program, "render_wire"), render_wire ? 1 : 0); // assigned each frame
...
function handle_key_down(event)
{
    ...
    if(event.keyCode == 72){ // h for D3
        render_wire = !render_wire;     // set grey colour enabled
        draw_mode = render_wire ? gl.LINES : gl.TRIANGLES;  // set draw mode 
    }
